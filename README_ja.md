...
このプロジェクトの主な貢献は、**洗練されたジェネラティブアートの数式を体系的に整理し、それを色立体視の効果と組み合わせた**点にあります。パーティクルや背景をユーザーが自由にカスタマイズできるようにすることで、この裸眼3D現象の発生をコントロールし、平面的な数学アートをダイナミックで立体的なビジュアル体験へと変化させるツールを提供します。

## 主な機能

### 🎨 多彩なアート表現
- **48種類のコードバリエーション**：それぞれ独自の数式に基づき、異なるビジュアルを生成します。
...
5. **画像の保存**：カメラアイコンをクリックすると、表示中のアートがPNG形式で保存されます。ファイル名は `generative-art-code20-2025-07-28_16-52-55.png` のように自動生成されます。

### パラメータガイド
- **コード選択**：48種類の数式アルゴリズムから、それぞれ異なるビジュアルを選択します。
